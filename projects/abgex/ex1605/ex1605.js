...
var glnCase = 0; // Case de placement de cartes
...
if (glnTime == glnTimeMax) {
  ...
  // Recherche la prochaine case contenant une carte  
  do {
    glnCase += 1;
    idImage = 'idPos' + (100 + glnCase);
    gloImage = document.getElementById(idImage + '1'); // carte de la case "glnCase" s'il y en a une
    // L'id termine par '1' pour la carte du dessus du tas    
  } while ( (glnCase <= gloB.nCaseMax) && (gloImage == null) )

  if (gloImage == null) {
    if (glnRepetition >= 1 && !gloB.fRunStop) runCode_slow(2, 10); // Exécute le code
    return; // fin des déplacement de cartes.
    }
  
  glnTime = 0; // initialisation pour le déplacement de la carte
  glnCardPosX1 = 100 + 100 * glnCase;
  }
...
for (nn=1; nn<=gloB.nCaseMax; nn++) { // Place les cartes
  if ( (nn != nCaseExclue_1) && (nn != nCaseExclue_2) ) {
    ...
    strInstr += " <img src='../images/Carte" + nCarte + ".png' alt='" + nCarte + "' width=80 height=128"
             ...
             + "\n style='position:absolute; left:" + glnCardPosX0 + "px; top:190px;  z-index:91;'>\n";
    }
  } // for
...
//=======================
// Ferme le dialogue
document.getElementById('idDialogWellSorted').style.display='none';

// Place les cartes et fait un tri                      
Place_cadres_cartes(0, 0, 0);
}

function Display_ok_and_resort() {
//================================
// Affiche que le tri c'est bien fait et fait un tri supplémentaire.
document.getElementById('idDialogWellSorted').style.display='block';
