...
var glnTimeNow; // Temps en millisecondes actuel
var glnTimeLast = 1e12; // Temps en millisecondes du dernier appel à 'myUpdateFunction'

function myUpdateFunction(event) {
//================================
// Met à jour le code automatiquement.
// J'aimerais savoir si une connexion a été refusée par incompatibilité de types.
// Je ne sais pas comment faire. 
// On peut tester la variation de coordonnées, mais c'est du bidouillage.
// c.f. https://developers.google.com/blockly/reference/js/Blockly.DraggedConnectionManager
// c.f. https://developers.google.com/blockly/reference/js/Blockly#.BUMP_DELAY
...
if (event.type == 'move') {
  // Fonctionne, c.f. https://developers.google.com/blockly/guides/configure/web/events
  //Display(event.oldCoordinate.x + ' .. ' + event.newCoordinate.x + '\n');   
  //Display(event.newInputName + '\n'); // == 'undefined' s'il n'y a pas d'accroche

  var oTime = new Date();
  glnTimeNow = oTime.getTime(); // Temps écoulé en millisecondes depuis le 1er janvier 1970.
  Display(event.type + ' .. ' + (glnTimeNow - glnTimeLast) + ' .. ' + glnTimeNow 
  ...
  // echo(DraggedConnectionManager); // Ne donne rien
  // var oDrag = new DraggedConnectionManager(glnObject);   Display(oDrag); // Ne donne rien
  //Display(oDrag.wouldConnectBlock() + '\n');


  // On peut estimer qu'il y a eu un rejet, si 
  // ° l'événement à eu lieu rapidement après le précédent
  // ° la variation de la position en  X  est comprise dans des bornes  ...
  // ° la variation de la position en  Y  est comprise dans des bornes  ...
  // Tous les rejets sont bien détectés, 
  // mais il est possible d'obtenir de fausses détections de rejet.
  var nDeltaX = event.newCoordinate.x - event.oldCoordinate.x;
  var nDeltaY = event.newCoordinate.y - event.oldCoordinate.y;
  if ( (glnTimeNow - glnTimeLast <= Blockly.BUMP_DELAY+Blockly.BUMP_RANDOMNESS+6)  
    && (nDeltaX < 60)
    && (nDeltaX > 10)
    && (nDeltaY < 60)
    && (nDeltaY > 10)
//    && (event.newCoordinate.x < 300) // trop limitatif
     ) {
    Display("************ REJET ******" + nDeltaX + ' .. ' + nDeltaY + ' .. ' + '\n');
    Blockly.BUMP_DELAY = 150; // Raccourcit le temps pris pour rejeter le block
    // Avec un temps < 150 [ms], il est très très difficile d'obtenir de fausses détection de rejets.
    Blockly.BUMP_RANDOMNESS = 1;
    }
  
  oTime = new Date();
  glnTimeLast = oTime.getTime(); // Mémorise l'heure du dernier appel.
  } // if (event.type == 'move') {
else {
  Display(event.type + '\n');
  }
...
if (glnTime >= nTimeMax) { // L'animation dure  nTimeMax  milli-secondes
  ...
  divElement = document.getElementById('idDivSon');    
  // c.f. https://developer.mozilla.org/fr/docs/Web/API/Node/removeChild
...
//""""""""""""""""""""""""""""""""""""
  init: function() {
    ...
    this.setOutput(true, "USB");  // on défini le type de sortie possible, qui est USB
    this.setColour(90);
...
function func_mouse_change(event) {
//=================================
// Appelée à chaque action sur la souris, si 'setOnChange' a été appelé dans le constructeur de la souris.
Display('mouse_change : ' + event.type +  '\n');
}
...
//""""""""""""""""""""""""""""""""""""
  init: function() {
    ...
    this.setOutput(true, "USB_MOUSE");  // on défini le type de sortie possible, qui est USB
    this.setColour(90);
    this.setTooltip("C'est une souris");
    this.setHelpUrl("");
    this.data = 'LIFE';
    // this.setOnChange(func_mouse_change);  défini une fonction appelée à chaque action sur la souris
  }
};
