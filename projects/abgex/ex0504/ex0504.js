...
var glanMemorise_Cartes = new Array(5+2);
...
//for (nn=1; nn<=5+1; nn++) { Display(nn + ' > ' + glanMemorise_Cartes[nn] + ' ; '); } Display('\n');
...
var idImage = 'idPos';
var anCase_Carte = new Array(5+2);
var oImage = null;
var fOK = true; // On part de l'hypothèse que les cartes ont bien été permuttées

// Enregistre le contenu des 5 cases
for (nFin=1; nFin<=5+1; nFin++) {
...
// En parcourrant les 5 cases, test de permuttation.
...
for (nFin=1; nFin<=5; nFin++) {
  if (anCase_Carte[nFin] == 0) {
    ...
    for (nInit=1; nInit<=5+1; nInit++) {
      if (glanMemorise_Cartes[nInit] == anCase_Carte[nFin]) break; // case trouvée, mise dans nInit
      }
    if (nInit > 5) { fOK = false; break; } // La carte était en dehors des 5 cases.
...
if (fOK) PlaySound('../medias/carill2.mp3', "Bravo, deux paires de cartes ont bien été permuttées");
else     PlaySound('../medias/chimes.mp3', "Les deux paires de cartes n'ont pas été permuttées");
} // TestReponse
