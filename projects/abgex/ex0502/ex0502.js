...
if (glnTimerID1 !== 0) {
  ...
  gloImageBouge.style.zIndex = 500;
  }
} // Stop_image
...
while (nProfondeur > 1) {
  ...
  oImage2.style.zIndex = 500 - nProfondeur;
  nProfondeur -= 1; // Remonte d'une carte
  }
...
var glanMemorise_Cartes = new Array(5+1);
...
//
// Lecture du contenu des 5 cases
for (nn=1; nn<=5; nn++) {
  idImage = 'idPos' + (100 + nn);
  oImage = document.getElementById(idImage + '1');
  // L'id termine par '1' pour la carte du dessus du tas
  if (oImage == null) {
    // Pas de carte à cette place
    if (glanMemorise_Cartes[nn] != 0) {// Il y a une carte où il n'y en avait pas au départ !
      fOK = false;
      }
    }
  else {
    // Il y a une carte à cette place.
    nCarteNum = parseInt(oImage.getAttribute("name"));
    if ( (glanMemorise_Cartes[nn] == nCarteNum) // Une carte est restée sur place
      || (glanMemorise_Cartes[nn] == 0) ) {// Il n'y avait pas de cartes à cet endroit au départ
      fOK = false;
      }
    }
  } // for

if (fOK) PlaySound('../medias/carill2.mp3', "Bravo, vos deux cartes ont bien été permuttées");
else     PlaySound('../medias/chimes.mp3', "Les deux cartes n'ont pas été permuttées");
} // TestReponse
...
code =  "MemoriseSituationInitiale();\n"
      + "function* Ralenti() {\n" + code 
      + "highlightBlock(null);\n"
      + "\nyield 0;\n}"
      + "\nglGen = Ralenti();\n"
