...
var glnRepetition = 0; // Pour répéter plusieurs fois l'algorithme. Toujours à 1 dans ce programme.

var glnCase = 25; // Case de placement de cartes
...
var glnCardPosY0 =  192;
var glnCardPosY1 =   18; // Fixe pour toutes les cartes
var glnTimeMax = 20; // Nombre de déplacement d'une carte pour la mettre en place
...
gloB.nCaseMax   =   25; // Nombre de cases
gloB.nCardsLeft  = 10; // Position de la case '0'
gloB.nCardsTop   = 120;  // Position du haut des cartes (en pixels)
gloB.nCardsDelta = 40; // Distance entre deux cases (en pixels)
gloB.nCardsShift =  10;  // Pour décaler les cartes se trouvant sur le même tas.
gloB.cHide = ""; // N'accepte pas la superposition de cartes. Si vide, accepte la superposition de cartes.
...
if (glnTime == glnTimeMax) {
  ...
  glnCardPosX1 = 12 + 40 * glanCases[glnCardNum]; // Case de placement de la carte
  }
...
setTimeout(Place_cartes_move, 5);
} // Place_cartes_move
...
var strInstr = '';
var nbrCartes = 25; // nombre de cartes
var nCase = 25; // case n°2, celle du milieu
...
for (nn=1; nn<=gloB.nCaseMax; nn++) { // Place les cases
  strInstr += " <img src='../images/Cadre_noire_2px.png' alt='cadre noire' width=29 height=47\n" 
           +  " style='position:absolute; left:" + (40 * nn + 10) + "px; top:15px; z-index:90;'>\n"
           +  " <span style='position:absolute; left:" + (40 * nn + 16) + "px; top:80px;  z-index:90;'>" + nn + "</span>\n";
  } // for
...
for (nn=1; nn<=nbrCartes; nn++) { // Place les cartes
    ...
    // Au départ, toutes les cartes présentent leur dos
    strCarteSrc = 900; 
    
    anDepth[nCase] = 1;
    glnCardTot += 1; // Compte le nombre de cartes sur le tas
    glanCases[glnCardTot] = nn; // Mémorise la case de la carte
    glanDepths[glnCardTot] = anDepth[nCase]; // Mémorise la profondeur de la carte dans le tas

    strInstr += " <img src='../images/Carte" + strCarteSrc + ".png' alt='Carte " + nCarte + "' width=25 height=40"
             +  "\n id='idPos" + (100 + nn) // Indique la position de la carte
             ...
             + "\n style='position:absolute; left:" + (glnCardPosX0) + "px; top:190px;  z-index:" + (200-nn) + ";'>\n";
...
gloB.nCaseMax = 25; // Numéro de la dernière case. Il y aura toujours 25 cartes.
...
//======================
// Test la validité du résultat, suivant le niveau
var nn = 0;
var nCardValue = 0; // Valeur visible de la carte
var oImage = null;
var fOK = true; // On part de l'hypothèse que l'algorithme est correct.

if (glnLevel == 1) {
  // Teste que toutes les cartes sont retournées
 
  for (nn=1; nn<=gloB.nCaseMax; nn++) {
    if (GetCardValue(nn) == 900) fOK = false;   // La carte de la case  nn  présente son dos    
    }
  } // if (glnLevel == 1) {
else if (glnLevel == 2) {
  // Teste que toutes les cartes des case paires sont retournées, pas les autres.
 
  for (nn=1; nn<=gloB.nCaseMax; nn++) {
    nCardValue = GetCardValue(nn); // Valeur visible de la carte
    if ( (nn % 2 == 0) && (nCardValue != 900) ) fOK = false;   // La carte de la case  nn  ne présente pas son dos    
    if (nn % 2 == 1) {
      if ( (nCardValue == 900) || (nCardValue == 0) ) fOK = false;   // La carte de la case  nn  présente son dos ou est absente
      }
    }
  } // if (glnLevel == 2) {
else if (glnLevel == 3) {
    // Teste que les cartes sont retournées comme demandé
    nCardValue = GetCardValue(1); // Valeur visible de la carte
    if ( (nCardValue == 900) || (nCardValue == 0) ) fOK = false;   // La carte de la case  nn  présente son dos ou est absente
 
  for (nn=2; nn<=gloB.nCaseMax; nn++) {
    nCardValue = GetCardValue(nn); // Valeur visible de la carte
    if ((nn-2) % 6 <= 2) {
      if (nCardValue != 900) fOK = false;   // La carte de la case  nn  ne présente pas son dos    
      }
    else {
      if ( (nCardValue == 900) || (nCardValue == 0) ) fOK = false;   // La carte de la case  nn  présente son dos ou est absente
      }
    ...
    // Teste que les cartes retournées sont dans des cases étant des carrés parfait.
 
  for (nn=1; nn<=gloB.nCaseMax; nn++) {
    nCardValue = GetCardValue(nn); // Valeur visible de la carte
    if ( (nn == 1) || (nn == 4) || (nn == 9) || (nn == 16) || (nn == 25) )  {
      if ( (nCardValue == 900) || (nCardValue == 0) ) fOK = false;   // La carte de la case  nn  présente son dos ou est absente
      }
    else {
      if (nCardValue != 900) fOK = false;   // La carte de la case  nn  ne présente pas son dos    
