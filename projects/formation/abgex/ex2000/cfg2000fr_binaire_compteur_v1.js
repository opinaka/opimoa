...
+'\n  </variables>'
+'\n  <block type="card_link_func_card" x="18" y="51">'
+'\n    <value name="FRAME1">'
...
+'\n    </next>'
+'\n  </block>'
+'\n  <block type="procedures_defnoreturn" x="27" y="249">'
...
+'\n    </statement>'
+'\n  </block>'
+'\n  <block type="procedures_defnoreturn" x="27" y="466">'
...
// Case pour un clic sur "précédent", non implémenté
nn += 1;
glaoFrames[nn].posX = 60;
glaoFrames[nn].posY = glaoFrames[1].posY + glaoFrames[1].deltaY + 45;
glaoFrames[nn].deltaX = 100;
glaoFrames[nn].deltaY = 30;
glaoFrames[nn].fWithNumber = false;

// Case pour un clic sur "suivent", implémenté
nn += 1;
glaoFrames[nn].posX = glaoFrames[nn-1].posX + glaoFrames[nn-1].deltaX + 40;
glaoFrames[nn].posY = glaoFrames[nn-1].posY;
glaoFrames[nn].deltaX = 100;
glaoFrames[nn].deltaY = 30;
glaoFrames[nn].fWithNumber = false;

// Pour y placer une carte avec un texte "Reset". Non implémenté
nn += 1;
glaoFrames[nn].posX = glaoFrames[nn-1].posX + glaoFrames[nn-1].deltaX + 40;
glaoFrames[nn].posY = glaoFrames[nn-1].posY;
glaoFrames[nn].deltaX = 100;
...
gloCanvas.nbCards = 8; // Nombre de cartes
for (nn=1; nn<=gloCanvas.nbCards; nn++) { // Pour créer des instances de cartes
    nCardFace = 1048; // Référence à l'image de la carte
    nCardBack = 1049;
    nCase = nn;
    if (nn >=  6) {nCardFace = 1032; nCardBack = 1032; } // Carte blanche.
    ...
    strInstr += " <img src='" + gloCanvas.strSource + nCardFace + gloCanvas.strExt + "' alt='" + nCardFace + "' width=80 height=128"
             ...
             +  "\n style='position:absolute; left:" + 70*nCase + "px; top:190px;  z-index:" + 500 + ";'>\n";
...
// Écriture de texte par dessus l'avant-dernière carte, celle qui est blanche
CardTextWrite(1, glaoFrames[7].posX + 5, glaoFrames[7].posY + 24, "Suivant");

// Écriture de texte par dessus la dernière carte, celle qui est blanche
glaoTexts[2].color = "#400000";
CardTextWrite(2, glaoFrames[8].posX + 5, glaoFrames[8].posY + 24, "Reset");

// Devra être ajouté pour l'utilisateur
glaoTexts[3].color = "#0000ff";
//CardTextWrite(3, glaoFrames[6].posX + 5, glaoFrames[6].posY + 24, "Précédent");

// Pour compter le nombre de fois que l'on a indiqué la carte choisie
// On stock ce nombre dans le paramètre associé à la carte de la case 8.
// 8 = la case, 1 = la profondeur = dessus, 'data-param1' est le nom du paramètre et '0' la valeur qu'on lui donne.
//CardSetParam(8, 1, 'data-param1', '0'); // Pas nécessaire
} // confCardsInit
...
+ "&nbsp; Exemple d'un compteur en binaire &nbsp;<br>"
+ "&nbsp; C'est la manière de compter en base 2. &nbsp;<br>"
+ "&nbsp; En base 2 : <b>1 + 1 = 10</b>.<br>"
+ "&nbsp;<br>"
+ "&nbsp; Le traitement du bouton 'Reset' est à faire. &nbsp;<br>"
+ "&nbsp;<br>"
+ "&nbsp; L'adjonction de texte sur le premier bouton est à faire. &nbsp;<br>"
+ "&nbsp; Une fonction 'précédent' peut être implémentée."
;
