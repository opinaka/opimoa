...
3 colonnes de 9 cartes sont disponibles, pour programmer un tour de magie ou faire autre chose.
 */
// ########################################################################################
...
+'\n    <value name="COMMENT">'
+'\n      <shadow type="text">'
+'\n        <field name="TEXT">Juste un exemple, à effacer.</field>'
+'\n      </shadow>'
+'\n    </value>'
+'\n    <next>'
...
// Trois cases pour les choix de colonnes
glaoFrames[nn-2].posX  += 22;
glaoFrames[nn-2].deltaX = 43;
glaoFrames[nn-2].deltaY = 67;
glaoFrames[nn-2].fWithNumber = false;
...
nn += 1; // = 31
glaoFrames[nn].posX = glaoFrames[nn-4].posX + glaoFrames[nn-4].deltaX;
glaoFrames[nn].posY = glaoFrames[nn-4].posY + glaoFrames[nn-4].deltaY + 30;
...
nn += 1; // = 32
glaoFrames[nn].posX = 90;
glaoFrames[nn].posY = 500;
...
gloCanvas.nbCards = 31; // Nombre de cartes
for (nn=1; nn<=gloCanvas.nbCards; nn++) { // Pour créer des instances de cartes
    nCardFace = 100 + Math.floor((nn-1)/3) + 100 * ((nn-1) % 3);
    nCase = nn;
    if (nn == 28) {nCardFace = 900; } // Carte bleue
    if (nn == 29) {nCardFace = 905; } // Carte verte
    if (nn == 30) {nCardFace = 910; } // Carte rouge.
    if (nn == 31) {nCardFace = 920; } // Carte blanche.
...
CardTextWrite(0, glaoFrames[31].posX+4, glaoFrames[31].posY+24, "Reset");

// Pour compter le nombre de fois que l'on a indiqué la carte choisie
// On stock ce nombre dans le paramètre associé à la carte de la case 28.
// 28 = la case, 1 = la profondeur = dessus, 'data-param1' est le nom du paramètre et '0' la valeur qu'on lui donne.
CardSetParam(28, 1, 'data-param1', '0');
} // confCardsInit
...
+ "&nbsp; Prépare une situation pour pgrogrammer un tour de magie avec 3 colonnes de 9 cartes. &nbsp;<br>"
+ "<br>"  // <br> est une balise pour sauter une ligne.  Toutes la synthaxe HTML est disponible. 
+ "&nbsp; C'est à vous d'écrire le programme.<br>"
+ "<br>"  // <br> est une balise pour sauter une ligne.  Toutes la synthaxe HTML est disponible. 
+ "&nbsp; Vous pouvez bien-sûr utiliser cette situation de départ pour le programme que vous désirez. &nbsp;<br>"
